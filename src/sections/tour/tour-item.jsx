...
import { fDateTime, fDateRangeShortLabel } from '~/utils/format-time';
...
export function TourItem({ tour, onView, onEdit, onDelete }) {
    ...
    const renderImages = (
        <Box gap={0.5} display="flex" sx={{ p: 1 }}>
            <Box flexGrow={1} sx={{ position: 'relative' }}>
                {renderPrice}
                {renderRating}
                <Image alt={tour.images[0]} src={tour.images[0]} sx={{ width: 1, height: 164, borderRadius: 1 }} />
            </Box>
    ...
    const renderTexts = (
        <ListItemText
            ...
            secondary={
                <Link component={RouterLink} href={paths.dashboard.kanban.details(tour.id)} color="inherit">
                    {tour.name}
                </Link>
            }
    ...
    const renderInfo = (
        <Stack spacing={1.5} sx={{ position: 'relative', p: (theme) => theme.spacing(0, 2.5, 2.5, 2.5) }}>
