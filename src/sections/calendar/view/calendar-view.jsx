...
export function CalendarView() {
    ...
    const { events, eventsLoading, mutateEvents } = useGetEvents();
    ...
    const canReset = filters.state.colors.length > 0 || (!!filters.state.startDate && !!filters.state.endDate);

    const dataFiltered = applyFilter({ inputData: events, filters: filters.state, dateError });

    const renderResults = (
        <CalendarFiltersResult filters={filters} totalResults={dataFiltered.length} sx={{ mb: { xs: 3, md: 5 } }} />
    );
    ...
    const updateEvent = useCallback(
        async (eventId, eventData) => {
            ...
            mutateEvents((events) =>
                events.map((event) => (event.id === +eventId ? { ...event, ...eventData, id: +eventId } : event)),
            );
        },
        [mutateEvents],
    );
