...
export function HomePricing({ sx, ...other }) {
    ...
    const renderContentMobile = (
        <Stack spacing={5} alignItems="center" sx={{ display: { md: 'none' } }}>
            ...
            >
                {PLANS.map((tab) => tab.license === tabs.value && <PlanCard key={tab.license} plan={tab} />)}
            </Box>
        </Stack>
    );
...
const PLANS = [...Array(3)].map((_, index) => ({
    ...
    options: ['JavaScript version', 'TypeScript version', 'Design resources (Figma)', 'Commercial applications'],
    icons: [
