...
export function TabsView() {
    ...
    const DEMO = [
            ...
            component: (
                <ComponentBlock sx={{ flexDirection: 'column', alignItems: 'unset' }}>
                    ...
                    <Paper variant="outlined" sx={{ p: 2.5, typography: 'body2', borderRadius: 1.5 }}>
                        {TABS.slice(0, 3).map((tab) =>
                            tab.value === basicTabs.value ? <Fragment key={tab.value}>{tab.label}</Fragment> : null,
                        )}
                    </Paper>
                </ComponentBlock>
            ),
        ...
        },
        {
            ...
            component: (
                <ComponentBlock>
                    <Tabs value={scrollableTabs.value} onChange={scrollableTabs.onChange} sx={{ maxWidth: 320 }}>
                        {TABS.map((tab) => (
