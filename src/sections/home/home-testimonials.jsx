...
import { maxLine, varAlpha, textGradient } from '~/theme/styles';

import { varFade, MotionViewport, AnimateCountUp } from '~/components/animate';
import {
    Carousel,
    useCarousel,
    CarouselDotButtons,
...
export function HomeTestimonials({ sx, ...other }) {
    ...
    const renderDescription = (
        <SectionTitle title={t('testimonials.title')} sx={{ mb: { xs: 5, md: 8 }, textAlign: 'center' }} />
    );
    ...
    const horizontalDivider = (position) => (
        <Divider
            ...
            sx={{
                ...
                border: 'none',
                position: 'absolute',
                background: `linear-gradient(to right, ${varAlpha(theme.vars.palette.grey['500Channel'], 0)} 0%, ${
                    theme.vars.palette.grey[500]
                } 50%, ${varAlpha(theme.vars.palette.grey['500Channel'], 0)} 100%)`,
                ...(position === 'top' && { top: 0 }),
    ...
    const verticalDivider = (
        <Divider
            ...
            sx={{
                ...
                border: 'none',
                width: '1px',
                background: `linear-gradient(to bottom, ${varAlpha(theme.vars.palette.grey['500Channel'], 0)} 0%, ${
                    theme.vars.palette.grey[500]
                } 50%, ${varAlpha(theme.vars.palette.grey['500Channel'], 0)} 100%)`,
            ...
            <Carousel carousel={carousel}>
                {TESTIMONIALS.map((item) => (
                    <Stack key={item.id} component={m.div} variants={varFade().in}>
                        ...
                        <Typography sx={{ ...maxLine({ line: 4, persistent: theme.typography.body1 }), mt: 2, mb: 3 }}>
                            {item.content}
                        </Typography>
    ...
    const renderNumber = (
        <Stack sx={{ py: { xs: 5, md: 8 }, position: 'relative' }}>
            ...
            <Stack spacing={5} direction={{ xs: 'column', md: 'row' }} divider={verticalDivider}>
                {[
                    { label: t('statistics.purchasedOrders'), value: 12.121 },
                    { label: t('statistics.happyCustomers'), value: 160 },
                    { label: t('statistics.reviewRate'), value: 4.9 },
                ].map((item) => (
                    <Stack key={item.label} spacing={2} sx={{ textAlign: 'center', width: 1 }}>
                        <m.div variants={varFade({ distance: 24 }).inUp}>
                            <AnimateCountUp
                                to={item.value}
                                unit={item.label === t('statistics.purchasedOrders') ? 'k+' : '+'}
                                toFixed={item.label === t('statistics.happyCustomers') ? 0 : 1}
                                sx={{
